...
int main(void) {
    ...
    //Habilitar interrupción del UART
    IntMasterEnable();
    //Habilitar ADC
...
void ObtenerMuestra(){           //Función para guardar valores del adc
    ADCIntClear(ADC0_BASE, 1);   //Clear en la bandera de interrupción del ADC
     a = ADC0_SSFIFO1_R;         //Obtención de datos usando FIFO
     b = ADC0_SSFIFO1_R;
     c = ADC0_SSFIFO1_R;
     d = ADC0_SSFIFO1_R;
}
...
void InitUART(void)
{
    ...
    /*Habilitar módulo 7 de UART*/
    SysCtlPeripheralEnable(SYSCTL_PERIPH_UART7);

    /*Configuración para utilizar los pines del UART7*/

    GPIOPinTypeUART(GPIO_PORTE_BASE, GPIO_PIN_0 | GPIO_PIN_1);
    GPIOPinConfigure(GPIO_PE0_U7RX);
    GPIOPinConfigure(GPIO_PE1_U7TX);

    /* Configuración general del UART*/
